...
sub startup {
  ...
  # );
  my $dbi = DBIx::Custom->connect(
    dsn => "dbi:Pg:database=gitdada;host=148.59.34.110",
    user => 'pronoor',
    password => 'pronoor231',
    connector => 1
  );
...

  {
    ...
    # Auto route
    {
      ...
      # Custom routes
      {
        ...
        $r->get('/getUsers' => sub {
          my $self = shift;
          my $getusers = $self->app->dbi->model('user')->select(
            ['id','row_id'],
            where => {status => 0, admin => 0},
          )->all;
          
          $self->render(json => $getusers);
        });
